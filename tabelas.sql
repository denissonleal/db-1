  ...
  CREATE OR REPLACE TYPE TP_PRODUTO
   AS
    ...
    (
     ...
     VALOR float(2),
     QUANTIDADE integer,
     CONSTRUCTOR FUNCTION TP_PRODUTO(X TP_PRODUTO) RETURN SELF AS RESULT,
     MEMBER PROCEDURE EXIBIR_DETALHES(SELF TP_PRODUTO),
     MAP MEMBER FUNCTION PRODUTOTOINT RETURN INTEGER
    );
--------------------------------------------------------------------------------
...
CREATE TABLE TB_VENDA(
  COD_VENDA INTEGER,
  FUNCIONARIO_LANCHONETE REF TP_LANCHONETE,
  CLIENTE REF TP_CLIENTE,
  LISTA_PRODUTOS TP_NT_PRODUTO,
  CONSTRAINT PK_VENDA PRIMARY KEY(COD_VENDA)
  ) NESTED TABLE LISTA_PRODUTOS STORE AS TB_LISTA_PRODUTO;
--------------------------------------------------------------------------------
...
CREATE OR REPLACE TYPE BODY TP_PRODUTO AS
     CONSTRUCTOR FUNCTION TP_PRODUTO(X TP_PRODUTO) RETURN SELF AS RESULT IS
      BEGIN
      ...
      END;
     MEMBER PROCEDURE EXIBIR_DETALHES(SELF TP_PRODUTO) IS
     BEGIN
      DBMS_OUTPUT.PUT_LINE('Detalhes de um Produto');
      DBMS_OUTPUT.PUT_LINE('CÓDIGO: ' || TO_CHAR(COD_PRODUTO));
     ...
     MAP MEMBER FUNCTION PRODUTOTOINT RETURN INTEGER IS
     P INTEGER := COD_PRODUTO;
     BEGIN
      RETURN P;
     END;
END;
